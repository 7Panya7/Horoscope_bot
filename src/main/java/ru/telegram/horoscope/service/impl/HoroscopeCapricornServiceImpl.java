...
import ru.telegram.horoscope.parser.Parser;
import ru.telegram.horoscope.config.BotState;
import ru.telegram.horoscope.parser.ParserHoroscopePage;
import ru.telegram.horoscope.service.HoroscopeCapricornService;

@Service
@RequiredArgsConstructor
public class HoroscopeCapricornServiceImpl implements HoroscopeCapricornService {
    ...
    public SendMessage handle(long chatId, Message message) {
        var text = parserHoroscopePage.getPage("capricorn");
        SendMessage sendMessage = new SendMessage(String.valueOf(message.getChatId()),text);
        sendMessage.setReplyMarkup(inlineKeyboardMarkup);
        return sendMessage;
    }
